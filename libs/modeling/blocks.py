...
class InstanceNorm(nn.Module):
    """
    InstanceNorm that supports inputs of size B, C, T
    """
    ...
    def forward(self, x, mask):
        ...
        # normalization along instances

        mask = mask.float() # (B,1,T)
        mean = (torch.sum(x * mask, 2) / torch.sum(mask, 2))   # (B, C)

        mean = mean.detach()
        var_term = ((x - mean.unsqueeze(2)) * mask)**2  # (B, C, T)

        var = (torch.sum(var_term, 2) / torch.sum(mask, 2))  #(B, C)

        var = var.detach()
        mean_reshaped = mean.unsqueeze(2)  # (B, C, 1)
        var_reshaped = var.unsqueeze(2)  # (B, C, 1)
        ins_norm = (x - mean_reshaped) / torch.sqrt(var_reshaped + self.eps)* mask.detach()  # (B, C, T)
...
class MaskedMHCCA(nn.Module):
    ...
    def forward(self, q, k_v, mask):
        ...
        q = self.query_norm(q)
        # key, value conv -> (B, nh * hs, T'')
        k, kv_mask = self.key_conv(k_v, mask)
        k = self.key_norm(k)
        v, _ = self.value_conv(k_v, mask)
        v = self.value_norm(v)
